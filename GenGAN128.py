    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            *discriminator_block(256, 512),
            nn.Conv2d(512, 1, 4, stride=1, padding=0),  # Adjusted output size
            nn.Sigmoid()  # Add Sigmoid to output a value between 0 and 1
        )
...
class GenGAN:
    def __init__(self, videoSke, loadFromFile=False):
        ...
        self.filename_G = 'DanceGenGAN2_G128.pth'
        self.filename_D = 'DanceGenGAN2_D128.pth'
    ...
    def train(self, n_epochs=100, patience=20):
        ...
        optimizer_G = optim.Adam(self.netG.parameters(), lr=0.0001, betas=(0.5, 0.999))
        optimizer_D = optim.Adam(self.netD.parameters(), lr=0.0004, betas=(0.5, 0.999))
        ...
        for epoch in range(n_epochs):
            for i, (ske_images, real_images) in enumerate(self.dataloader):
                ...
                valid = torch.ones(ske_images.size(0), 1, 1, 1, requires_grad=False).to(self.device)  # Adjusted size
                fake = torch.zeros(ske_images.size(0), 1, 1, 1, requires_grad=False).to(self.device)  # Adjusted size
